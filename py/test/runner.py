...
class RepublicmagTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            RepublicmagTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = RepublicmagTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = RepublicmagTestRunner.getenv("REPUBLICMAG_TEST_LIVE")
        override = RepublicmagTestRunner.getenv("REPUBLICMAG_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = RepublicmagTestRunner.getenv(key)
        ...
        explain = RepublicmagTestRunner.getenv("REPUBLICMAG_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["REPUBLICMAG_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if RepublicmagTestRunner._test_control is not None:
            return RepublicmagTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                RepublicmagTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            RepublicmagTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = RepublicmagTestRunner.load_test_control()
...
def load_env_local():
    RepublicmagTestRunner.load_env_local()


def env_override(m):
    return RepublicmagTestRunner.env_override(m)


def entity_data(v):
    return RepublicmagTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return RepublicmagTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return RepublicmagTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return RepublicmagTestRunner.load_test_control()


def live_client_options():
    return RepublicmagTestRunner.live_client_options()


def live_delay_ms():
    return RepublicmagTestRunner.live_delay_ms()
