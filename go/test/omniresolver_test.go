...
import (
	...
	omni "github.com/voxgig-sdk/republicmag-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.RepublicmagError); is {
				out := map[string]any{
					"name":    "RepublicmagError",
...
func omniCtx(arg any, client *sdk.RepublicmagSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.RepublicmagSDK); is {
			client = live
