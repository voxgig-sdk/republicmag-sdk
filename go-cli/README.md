...
boru-driven command-line client **and** interactive REPL for the Republicmag
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/republicmag-cli)
make build

# 2. See usage (words, entities, env vars)
./republicmag-cli --help

# 3. Provide credentials once, via the environment
export REPUBLICMAG_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./republicmag-cli list post

# 5. Override the API base URL for a single call
REPUBLICMAG_BASE=https://api.example.com ./republicmag-cli list post

# 6. No arguments -> interactive REPL
./republicmag-cli
republicmag> list post
republicmag> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/republicmag-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/republicmag-cli list post
   ```
...
Run with no arguments to open a REPL (prompt `republicmag>`). Each line is
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/republicmag-cli`. |
...
(`./republicmag-cli <expr>`) and interactively (the REPL), and expressions compose the
