// Command Republicmag-cli is a CLI + REPL for the Republicmag SDK,
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      REPUBLICMAG_APIKEY (api key), REPUBLICMAG_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: REPUBLICMAG_APIKEY carries the API key and
	// REPUBLICMAG_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("REPUBLICMAG_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("REPUBLICMAG_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewRepublicmagSDK(opts)
