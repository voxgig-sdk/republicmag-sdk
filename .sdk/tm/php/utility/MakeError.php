...
// Republicmag SDK utility: make_error

require_once __DIR__ . '/../core/Operation.php';
require_once __DIR__ . '/../core/Result.php';
require_once __DIR__ . '/../core/Error.php';

class RepublicmagMakeError
{
    public static function call(?RepublicmagContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new RepublicmagContext([], null);
        }
        $op = $ctx->op ?? new RepublicmagOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new RepublicmagResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof RepublicmagError) ? $err->msg : (string)$err;
        $msg = "RepublicmagSDK: {$opname}: {$errmsg}";
        ...
        $sdk_err = new RepublicmagError('', $msg, $ctx);
        ...
        if ($err instanceof RepublicmagError) {
