...
{
    private static function utility(): RepublicmagUtility
    {
        return new RepublicmagUtility();
    }
    ...
    private static function ctx(array $over = []): RepublicmagContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new RepublicmagContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new RepublicmagOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof RepublicmagError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new RepublicmagOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = RepublicmagMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new RepublicmagOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = RepublicmagMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new RepublicmagOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = RepublicmagMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new RepublicmagSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = RepublicmagMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new RepublicmagResponse([]),
            'result' => new RepublicmagResult([]),
        ]);
        [, $err] = RepublicmagMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new RepublicmagSpec([]),
            'response' => null,
            'result' => new RepublicmagResult([]),
        ]);
        [, $err] = RepublicmagMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new RepublicmagSpec(['step' => 's']),
            'response' => new RepublicmagResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new RepublicmagResult([]),
        ]);
        [, $err] = RepublicmagMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new RepublicmagSpec(['step' => 's']),
            'response' => new RepublicmagResponse(self::resp(200, ['v' => 1])),
            'result' => new RepublicmagResult([]),
        ]);
        [, $err] = RepublicmagMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new RepublicmagSpec(['step' => 's']),
            'response' => new RepublicmagResponse(self::resp(200, ['v' => 2])),
            'result' => new RepublicmagResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        RepublicmagMakeResponse::call($ctx);
    ...
    {
        $preset = new RepublicmagResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new RepublicmagSpec([]),
            'response' => new RepublicmagResponse([]),
            'result' => new RepublicmagResult([]),
        ...
        [$got, $err] = RepublicmagMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new RepublicmagResult([])]);
        [, $err] = RepublicmagMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new RepublicmagSpec([]), 'result' => null]);
        [, $err] = RepublicmagMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new RepublicmagSpec(['step' => 's']),
            'result' => new RepublicmagResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new RepublicmagOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = RepublicmagMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new RepublicmagSpec(['step' => 's']),
            'result' => new RepublicmagResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new RepublicmagOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = RepublicmagMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new RepublicmagResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new RepublicmagSpec([]),
            'result' => new RepublicmagResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = RepublicmagMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = RepublicmagMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new RepublicmagError('boom', 'boom');
        $utility->fetcher = function (RepublicmagContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new RepublicmagSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = RepublicmagMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (RepublicmagContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = RepublicmagMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(RepublicmagResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        RepublicmagMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (RepublicmagContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new RepublicmagSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = RepublicmagMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new RepublicmagResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new RepublicmagSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = RepublicmagMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = RepublicmagMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = RepublicmagMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new RepublicmagResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, RepublicmagDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new RepublicmagResult(['ok' => false])]);
        $this->expectException(RepublicmagError::class);
        RepublicmagDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new RepublicmagResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', RepublicmagMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        RepublicmagMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new RepublicmagBaseFeature();
        $b = new RepublicmagBaseFeature();
        RepublicmagFeatureAdd::call($ctx, $a);
        RepublicmagFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): RepublicmagBaseFeature
    {
        $f = new RepublicmagBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        RepublicmagFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): RepublicmagContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new RepublicmagSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = RepublicmagPrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_a_raw_apikey_goes_in_as_is(): void
    {
        ...
        RepublicmagPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new RepublicmagResponse(['headers' => null]),
            'result' => new RepublicmagResult([]),
        ]);
        RepublicmagResultHeaders::call($ctx);
    ...
    public function test_result_body_skips_parsing_when_the_body_is_absent(): void
    {
        ...
        RepublicmagResultBody::call($ctx);
