...
class FtCtrl extends RepublicmagControl
{
...
class FtClient
{
    ...
    private RepublicmagUtility $utility;

    public function __construct(RepublicmagUtility $utility, array $options)
    {
    ...
    public function get_utility(): RepublicmagUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (RepublicmagContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public RepublicmagContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (RepublicmagContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = RepublicmagConfig::make_feature($name);
    ...
    private static function buildUrl(RepublicmagSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof RepublicmagError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new RepublicmagResponse($fetched);
        }
    ...
    private function fail(RepublicmagContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (RepublicmagContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
