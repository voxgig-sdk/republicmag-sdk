...
module RepublicmagUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = RepublicmagResponse.new({})
    ...
    if fetch_err
      ...
      response = RepublicmagResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = RepublicmagResponse.new(fetched)
    else
