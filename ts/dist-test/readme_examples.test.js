...
function isRunnable(code) {
    return (/new\s+RepublicmagSDK\b/.test(code) ||
        /\bRepublicmagSDK\.test\b/.test(code) ||
        /\bclient\s*\./.test(code));
}
...
function rewriteForRun(code) {
    ...
    out = out.replace(/new\s+RepublicmagSDK\s*\([^)]*\)/g, () => SEEDED_CTOR);
    out = out.replace(/RepublicmagSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR);
