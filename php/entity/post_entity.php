...
class PostEntity
{
    ...
    {
        if ($args) {
            $this->_data = RepublicmagHelpers::to_map(Struct::clone($args)) ?? [];
    ...
    {
        if ($args) {
            $this->_match = RepublicmagHelpers::to_map(Struct::clone($args)) ?? [];
     ...
     *   the SDK boundary; throws RepublicmagError on failure (item-5 convention).
     */
