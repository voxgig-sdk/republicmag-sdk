...
// Republicmag SDK struct utility test
//
...
class StructUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = RepublicmagOmni::makeStructRunner(
        ...
        $this->runsection(
            ...
            function ($input) {
                ...
                if (is_object($store) && $haspath && Struct::undef() !== $val) {
                    ...
                    if (json_encode(RepublicmagOmni::structfix($now, Struct::undef()))
                        !== json_encode(RepublicmagOmni::structfix($val, Struct::undef()))) {
