...
[MCP](https://modelcontextprotocol.io) server exposing the Republicmag SDK as
two agent tools — `republicmag_list` and `republicmag_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/republicmag-mcp)
make build

# 2. Provide credentials via the environment
export REPUBLICMAG_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user republicmag \
  -- /absolute/path/to/republicmag-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./republicmag-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// republicmag_list: first page of records
{ "entity": "post" }
{ "entity": "post", "query": { } }

// republicmag_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/republicmag-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user republicmag \
     -- "$PWD"/dist/*/republicmag-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `republicmag_list` and `republicmag_load` tools now appear
   in new sessions. Ask the agent to *"list post using republicmag"*
   and it calls `republicmag_list` with `{"entity":"post"}`.
...
```sh
export REPUBLICMAG_APIKEY=sk_live_xxx            # API key
export REPUBLICMAG_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `republicmag_list` tool
...
### Call the `republicmag_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `republicmag_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `republicmag_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `REPUBLICMAG_APIKEY` | API key sent with every request. |
| `REPUBLICMAG_BASE` | Optional override of the API base URL. |
...
```sh
./republicmag-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"republicmag_load","arguments":{"entity":"post","query":{"id":1}}}}'
```
