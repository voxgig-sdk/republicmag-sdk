...
require_relative 'Republicmag_types'


class RepublicmagSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = RepublicmagHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = RepublicmagHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, RepublicmagFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    RepublicmagUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = RepublicmagHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = RepublicmagHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = RepublicmagHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => RepublicmagError.new(
        "#{op}_allow",
        "RepublicmagSDK: #{op}: operation not allowed by" \
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = RepublicmagHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = RepublicmagError.new(
        "graphql_error", "RepublicmagSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = RepublicmagSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
