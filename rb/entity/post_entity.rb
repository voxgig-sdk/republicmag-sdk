...
class PostEntity
  ...
  def data_set(args)
    if args
      @_data = RepublicmagHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = RepublicmagHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  # @return [Array<Post>, Array] the matching Post items; raises RepublicmagError on failure
