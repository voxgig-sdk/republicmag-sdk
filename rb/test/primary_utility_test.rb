...
class PrimaryUtilityTest < Minitest::Test
  ...
  def setup
    @runner = RepublicmagOmni.make_runner(TEST_JSON_FILE, RepublicmagSDK.test(nil, nil))
  ...
  def err_from_map(m)
    ...
    RepublicmagError.new(m["code"] || "", msg)
  end
  ...
  def test_make_point_basic
    ...
    subject = lambda do |ctx|
      ...
      err.nil? ? point : RepublicmagOmni.errify(err)
    end
  ...
  def test_operator_basic
    subject = lambda do |vin|
      vin = {} unless vin.is_a?(Hash)
      op = RepublicmagOperation.new(vin)
      {
        "entity" => op.entity,
        "name" => op.name,
        "input" => op.input,
