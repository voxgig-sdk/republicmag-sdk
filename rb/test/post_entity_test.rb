...
class PostEntityTest < Minitest::Test
  def test_create_instance
    testsdk = RepublicmagSDK.test(nil, nil)
  ...
  def test_stream
    ...
    base = RepublicmagSDK.test(seed, nil)
    seen = base.Post(nil).stream("list", nil, nil).to_a
    assert_equal 3, seen.length

    # Inbound: streaming active -> yields each item from the feature.
    cfg = RepublicmagConfig.shared_config
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      sdk = RepublicmagSDK.test(seed, { "feature" => { "streaming" => { "active" => true } } })
      got = []
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set REPUBLICMAG_TEST_POST_ENTID JSON to run live"
      return
    end
    client = setup[:client]
...
def post_basic_setup(extra)
  ...
  client = RepublicmagSDK.test(options, extra)
  ...
  entid_env_raw = ENV["REPUBLICMAG_TEST_POST_ENTID"]
  ...
  idmap_resolved = Helpers.to_map(
    env["REPUBLICMAG_TEST_POST_ENTID"])
  if idmap_resolved.nil?
    idmap_resolved = Helpers.to_map(idmap)
  end

  if env["REPUBLICMAG_TEST_LIVE"] == "TRUE"
    ...
    client = RepublicmagSDK.new(Helpers.to_map(merged_opts))
  end

  live = env["REPUBLICMAG_TEST_LIVE"] == "TRUE"
  {
    client: client,
    data: entity_data,
    idmap: idmap_resolved,
    env: env,
    explain: env["REPUBLICMAG_TEST_EXPLAIN"] == "TRUE",
    live: live,
