...
module RepublicmagUtilities
  MakeError = ->(ctx, err) {
    if ctx.nil?
      ...
      ctx = RepublicmagContext.new({}, nil)
    end
    op = ctx.op || RepublicmagOperation.new({})
    opname = op.name
    opname = "unknown operation" if opname.empty? || opname == "_"

    result = ctx.result || RepublicmagResult.new({})
    result.ok = false

    err = result.err if err.nil?
    err = ctx.make_error("unknown", "unknown error") if err.nil?

    errmsg = err.is_a?(RepublicmagError) ? err.msg : err.to_s
    ...
    sdk_err = RepublicmagError.new("", msg, ctx)
    ...
    sdk_err.code = err.code if err.is_a?(RepublicmagError)
